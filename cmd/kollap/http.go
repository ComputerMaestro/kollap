...
import (
	...
	documentssvr "github.com/ComputerMaestro/kollap/gen/http/documents/server"
	workspacessvr "github.com/ComputerMaestro/kollap/gen/http/workspaces/server"
...
func handleHTTPServer(ctx context.Context, u *url.URL, workspacesEndpoints *workspaces.Endpoints, documentsEndpoints *documents.Endpoints, wg *sync.WaitGroup, errc chan error, dbg bool) {
	...
	var (
		...
		documentsServer  *documentssvr.Server
	)
	{
		eh := errorHandler(ctx)
		workspacesServer = workspacessvr.New(workspacesEndpoints, mux, dec, enc, eh, nil)
		documentsServer = documentssvr.New(documentsEndpoints, mux, dec, enc, eh, nil)
	}

	// Configure the mux.
	workspacessvr.Mount(mux, workspacesServer)
	documentssvr.Mount(mux, documentsServer)
	...
	for _, m := range documentsServer.Mounts {
		log.Printf(ctx, "HTTP %q mounted on %s %s", m.Method, m.Verb, m.Pattern)
	}
